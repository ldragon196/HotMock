...
# Reference https://github.com/pimoroni/ioe-python
#
...
class IOE():
    ...
    def i2c_read8(self, reg):
        ...
        read = self._i2c_dev.i2c_read_block_data(self._i2c_addr, reg, 1)
        return read[0]

    def i2c_write8(self, reg, value):
        """Write a single (8bit) register to the device."""
        self._i2c_dev.i2c_write_block_data(self._i2c_addr, reg, [value])
...
RGB_ENCODER_I2C_ADDR = 0x0F  # 0x18 for IO Expander, 0x0F for the encoder breakout
...
class RGBEncoder:
    def __init__(self, i2c):
        self.__ioe = IOE(i2c = i2c, i2c_addr = RGB_ENCODER_I2C_ADDR, interrupt_pin = 4)
        self.__ioe.enable_interrupt_out(pin_swap = True)
        self.__ioe.setup_rotary_encoder(1, POT_ENC_A, POT_ENC_B, pin_c = POT_ENC_C)

        self.__ioe.set_pwm_period(PERIOD)
        self.__ioe.set_pwm_control(divider=2)  # PWM as fast as we can to avoid LED flicker

        self.__ioe.set_mode(PIN_RED, PWM, invert=True)
        self.__ioe.set_mode(PIN_GREEN, PWM, invert=True)
        self.__ioe.set_mode(PIN_BLUE, PWM, invert=True)
    
    # Set RGB led
    def set_rgb_ouput(self, r, g, b):
        self.__ioe.output(PIN_RED, r)
        self.__ioe.output(PIN_GREEN, g)
        self.__ioe.output(PIN_BLUE, b)

    # Get rgb encode value
    # Return h (in hsv with s = 1 v = 1)
    #        r, g, b
    def get_rgb_encode(self):
        count = 0
        if self.__ioe.get_interrupt():
            count = self.__ioe.read_rotary_encoder(1)
            self.__ioe.clear_interrupt()
...
# rgb.set_rgb_ouput(255, 0, 0)
# rgb.set_rgb_ouput(0, 255, 0)
# rgb.set_rgb_ouput(0, 0, 255)

while True:
    h, r, g, b = rgb.get_rgb_encode()
    print(h, r, g, b)
    rgb.set_rgb_ouput(r, g, b)
